/* ============================================================
   ULAR TANGGA SMPN 2 KAMAL — game.js
   Frontend murni untuk tampilan & DOM. Semua KEPUTUSAN permainan
   (dadu, gerak, cek jawaban, efek misteri, papan) diminta ke
   backend Python lewat fetch() ke /api/...
   ============================================================ */

/* ============================================================
   ASSET SLOT — dipanggil lewat onerror="assetSlot(this)" di HTML.
   Kalau file gambar belum ada di static/assets/, <img> ini otomatis
   berubah jadi kotak putus-putus "siap diisi". Begitu file dengan
   nama yang sesuai ditaruh di static/assets/, refresh halaman dan
   gambar itu langsung muncul menggantikan kotak ini — tidak perlu
...
/* ---------------- Pengaturan (tetap di localStorage, cuma preferensi tampilan) ---------------- */
...
/* ---------------- Audio: efek klik & backsound ----------------
   ...
   dari pengguna, jadi playBgm() sengaja baru dipanggil di dalam
...
function renderConnectors() {
  ...
  function placeConnector(tileA, tileB, imgSrc) {
    ...
    const angle = Math.atan2(dy, dx) * 180 / Math.PI + 270; // tileA = ujung "atas" gambar
    const thickness = Math.max(12, a.width * 0.34);
    const midX = (ax + bx) / 2, midY = (ay + by) / 2;

    const img = document.createElement('img');
    img.src = '/static/assets/' + imgSrc;
...
/* ---------------- Dadu 3D ---------------- */
// Sudut rotasi kubus (rotateX, rotateY) supaya sisi bernilai N menghadap layar.
// Diturunkan dari posisi tiap .face-N di CSS (lihat style.css) — bukan tebakan.
const DICE_TARGET = {
  1: { x: 0, y: 0 },
  2: { x: 0, y: -90 },
  3: { x: 90, y: 0 },
...
function spinDiceTo(n) {
  ...
  diceAccX = baseX + 720 + t.x;  // 2 putaran penuh + posisi akhir
  diceAccY = baseY + 1080 + t.y; // 3 putaran penuh + posisi akhir
...
async function rollDice() {
  ...
  try {
    // dadu benar-benar dilempar oleh Python: random.randint di lempar_dadu()
    const hasil = await apiSend('/api/lempar-dadu', 'POST');
...
/* ---------------- Pergerakan & resolusi petak (via Python) ---------------- */
async function movePawn(team, steps) {
  const hasil = await apiSend('/api/gerak', 'POST', { posisi: state.pos[team], langkah: steps });
  const tujuanAkhir = hasil.posisi_baru;
  await walkStepByStep(team, tujuanAkhir);
  updateSidebarUI();
  setTimeout(() => resolveTile(team, hasil.petak), 300);
}

/* Pion berjalan 1 petak per langkah (bukan langsung lompat ke tujuan),
   dengan jeda + bunyi klik kayu di tiap perpindahan 1 petak. */
...
/* ---------------- Modal: Petak Soal ---------------- */
async function openSoalModal(team) {
  const bank = await apiGet('/api/soal');
...
async function answerSoal(team, q, chosenIdx, optWrap) {
  // Python yang memutuskan benar/salah & berapa langkah hadiahnya
  const hasilJawab = await apiSend('/api/jawab', 'POST', { tipe: 'soal', id: q.id, pilihan: chosenIdx });
...
/* ---------------- Modal: Petak Bonus ---------------- */
async function openBonusModal(team) {
  const bank = await apiGet('/api/bonus');
  const q = bank[Math.floor(Math.random() * bank.length)];
...
async function answerBonus(team, q, chosenIdx, optWrap) {
  const hasilJawab = await apiSend('/api/jawab', 'POST', { tipe: 'bonus', id: q.id, pilihan: chosenIdx });
...
/* ---------------- Modal: Petak Misteri (diacak sistem, bukan dipilih pemain) ---------------- */
...
async function runMisteri(team) {
  const opponent = team === 'merah' ? 'biru' : 'merah';
  const hasil = await apiSend('/api/misteri', 'POST', {
    posisi_lawan: state.pos[opponent], shield_lawan: state.shield[opponent]
  });
  // jeda sejenak biar animasi "mengacak" kelihatan sebelum hasil dibuka
  setTimeout(() => {
    const cardSerang = document.getElementById('misteri-serang');
    const cardShield = document.getElementById('misteri-shield');
...
async function startNewGame() {
  try {
    state.board = await apiGet('/api/papan');
  } catch (e) {
    showToast('Gagal terhubung ke server Python. Pastikan "python app.py" berjalan.', 'merah');
    return;
  }
